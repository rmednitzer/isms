---
doc_id: REC-2026-510
doc_type: record
title: "Implementation statement for A.5.10 Acceptable use of information and other associated assets"
revision: 1
status: draft
approved_date: null
approved_by: null
owner: role:ISMS-Manager
classification: internal
supersedes_revision: null
next_review: "2027-01-01"
language: en
framework_refs:
  - iso27002:A.5.10
signature_ref: null
---

# Implementation statement for A.5.10 Acceptable use of information and other associated assets

**REC-2026-510 Revision 1 (DRAFT)**

*Status: draft. Owner: role:ISMS-Manager. Next review: 2027-01-01.*

## Control

A.5.10 Acceptable use of information and other associated assets (theme: organisational).

## Implementation

TODO: describe how {{entity.short_name}} implements this control.

## Evidence

TODO: reference evidence tasks (ET-NNN) bound to this control in `controls/evidence-plan.yaml`.

## Responsible role

TODO: role accountable for operating this control.

## Revision history

| Rev | Date | Status | Change |
|---|---|---|---|
| 1 | {{entity.draft_date}} | draft | initial draft |
