---
doc_id: REC-2026-528
doc_type: record
title: "Implementation statement for A.5.28 Collection of evidence"
revision: 1
status: draft
approved_date: null
approved_by: null
owner: role:ISMS-Manager
classification: internal
supersedes_revision: null
next_review: "2027-01-01"
language: en
framework_refs:
  - iso27002:A.5.28
signature_ref: null
---

# Implementation statement for A.5.28 Collection of evidence

**REC-2026-528 Revision 1 (DRAFT)**

*Status: draft. Owner: role:ISMS-Manager. Next review: 2027-01-01.*

## Control

A.5.28 Collection of evidence (theme: organisational).

## Implementation

Evidence relating to incidents is identified, collected, and preserved with chain of custody, per P-005.

## Evidence

No dedicated automated evidence task is bound to this control yet; evidence is captured manually per SOP-011 and the binding is tracked in `controls/evidence-plan.yaml`.

## Responsible role

role:CISO

## Revision history

| Rev | Date | Status | Change |
|---|---|---|---|
| 1 | {{entity.draft_date}} | draft | initial draft |
