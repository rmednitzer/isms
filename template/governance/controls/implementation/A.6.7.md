---
doc_id: REC-2026-607
doc_type: record
title: "Implementation statement for A.6.7 Remote working"
revision: 1
status: draft
approved_date: null
approved_by: null
owner: role:ISMS-Manager
classification: internal
supersedes_revision: null
next_review: "2027-01-01"
language: en
framework_refs:
  - iso27002:A.6.7
signature_ref: null
---

# Implementation statement for A.6.7 Remote working

**REC-2026-607 Revision 1 (DRAFT)**

*Status: draft. Owner: role:ISMS-Manager. Next review: 2027-01-01.*

## Control

A.6.7 Remote working (theme: people).

## Implementation

Security measures for remote working protect information accessed, processed, or stored outside the organisation's premises, per P-009 and P-002.

## Evidence

No dedicated automated evidence task is bound to this control yet; evidence is captured manually per SOP-301 and the binding is tracked in `controls/evidence-plan.yaml`.

## Responsible role

role:ISMS-Manager

## Revision history

| Rev | Date | Status | Change |
|---|---|---|---|
| 1 | {{entity.draft_date}} | draft | initial draft |
