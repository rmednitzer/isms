---
doc_id: REC-2026-710
doc_type: record
title: "Implementation statement for A.7.10 Storage media"
revision: 1
status: draft
approved_date: null
approved_by: null
owner: role:ISMS-Manager
classification: internal
supersedes_revision: null
next_review: "2027-01-01"
language: en
framework_refs:
  - iso27002:A.7.10
signature_ref: null
---

# Implementation statement for A.7.10 Storage media

**REC-2026-710 Revision 1 (DRAFT)**

*Status: draft. Owner: role:ISMS-Manager. Next review: 2027-01-01.*

## Control

A.7.10 Storage media (theme: physical).

## Implementation

Storage media are managed through their lifecycle of acquisition, use, transportation, and disposal per classification, per P-008 and P-010.

## Evidence

No dedicated automated evidence task is bound to this control yet; evidence is captured manually per SOP-301 and the binding is tracked in `controls/evidence-plan.yaml`.

## Responsible role

role:ISMS-Manager

## Revision history

| Rev | Date | Status | Change |
|---|---|---|---|
| 1 | {{entity.draft_date}} | draft | initial draft |
